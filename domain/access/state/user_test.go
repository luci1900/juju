...
func (s *userStateSuite) TestLastModelLogins(c *tc.C) {
	...
	c.Assert(err, tc.ErrorIsNil)
	c.Check(logins[coremodel.UUID(modelUUID1)].UTC(), tc.Equals, expectedTime1.Truncate(time.Second).UTC())
	c.Check(logins[coremodel.UUID(modelUUID2)].UTC(), tc.Equals, expectedTime2.Truncate(time.Second).UTC())
	...
	c.Assert(err, tc.ErrorIsNil)
	c.Check(logins[coremodel.UUID(modelUUID1)].UTC(), tc.Equals, expectedTime2.Truncate(time.Second).UTC())
	_, ok := logins[coremodel.UUID(modelUUID2)]
