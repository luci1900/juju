...
type ConnRequestState interface {
	...
	// model-scoped SSH connection request state.
	InsertSSHConnRequest(ctx context.Context, args domainssh.SSHConnRequest) error
}
...
func (tt *Tracker) RequestTunnel(ctx context.Context, req RequestArgs) (*gossh.Client, error) {
	...
	err = tt.connRequests.InsertSSHConnRequest(ctx, domainReq)
	if err != nil {
		return nil, err
	}
