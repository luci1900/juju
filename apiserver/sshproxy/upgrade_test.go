...
// serverWriteRaw starts a goroutine that reads (and discards) the request
// line and headers off the server side of conn, then writes raw bytes back.
// It does not close serverSide: net.Pipe has no internal buffering, so
// closing immediately after the write would race with the client still
// reading those same bytes. The caller owns serverSide's lifetime.
